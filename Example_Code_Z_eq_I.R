# Data
load("Example_Data_Z_eq_I.Rdata")
...
# CV Fit Function for LSMTR
CVfit_LSMTR <- function(filtered_indep,filtered_resp,group,maxgrid,gridlength=50,ite_ADMM=100){
  ...
  clusterExport(cl, c("ADMM_3block", "progress_bar", "hadamard.prod"))
  registerDoParallel(cl)
  chunk=foreach(i = 1:length(grid), .combine= 'cbind') %dopar% {
    CV_error_SLMTR2=rep(0,5)
    for(m in 1:5){
      temp <- ADMM_3block(initial_beta_0[,m],initial_beta[,,m],X_GRP[X_GRP[,1]!=m,][,-1],Y_GRP[Y_GRP[,1]!=m,][,-1],record_1[i,1]*sqrt(k),record_1[i,1],0.1,ite_ADMM,3)
      tempbeta <- as.matrix(as.data.frame(temp[4]))
      tempbeta_0 <- as.matrix(as.data.frame(temp[1]))
      CV_error_SLMTR2[m] <- mean((Y_GRP[Y_GRP[,1]==m,][,-1]-rep(1,dim(Y_GRP[Y_GRP[,1]==m,][,-1])[1])%*%t(tempbeta_0)-X_GRP[X_GRP[,1]==m,][,-1]%*%tempbeta)^2)
      m <- m+1 
    }
    c(mean(CV_error_SLMTR2))
  }
  stopCluster(cl)
  record_1[,2] <- as.vector(chunk)
  rec <- as.data.frame(record_1)
  colnames(rec) <- c("lambda_2","CV_Error")

  opt_lambda_2 <- rec[order(rec$CV_Error),][1,1]
  ...
  Sol_SLMTR_2 <- ADMM_3block(initial_0,initialSol,X,Y,opt_lambda_2*sqrt(k),opt_lambda_2,0.1,ite_ADMM*10,3)
  SS_SLMTR_2 <- as.matrix(as.data.frame(Sol_SLMTR_2[4]))
  ...
  colnames(SS_SLMTR_2) <- drugname[-11]
  row.names(SS_SLMTR_2) <- as.numeric(gsub("V",0,colnames(filtered_indep)))
